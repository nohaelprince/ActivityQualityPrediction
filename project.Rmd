...
```{r, echo=TRUE, cache=TRUE}
#library(randomForest); library(kernlab); library(caret)
...
# print the 10 top important features
vimp$var[1:10]

# plot the 10 top important features
barplot(vimp$Overall[1:10], main="Top 10 Important Features", horiz=TRUE, col=rainbow(10), 
...
```{r, echo=TRUE, cache=TRUE}
traing_pred <- predict(model, trainingdata)# We build the model using 5-fold cross validation.
